...
import type { GetTransactionHistoryType } from "@/app/api/transactions/route";
...
export default function TransactionTable({ from, to }: Props) {
  ...
  const query = useQuery<GetTransactionHistoryType>({
    ...
    queryFn: () =>
      fetch(
        `/api/transactions?from=${dateToUTCDate(from).toISOString()}&to=${dateToUTCDate(to).toISOString()}`,
